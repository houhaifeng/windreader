...
import cn.wind.com.page.mine.utils.DownloadURL;
...
public class TextExtractor {
  ...
  private final static String _keyWordsPattern = "(?<=meta name=\"keywords\" content=\").*?(?=\")";
  
  private final static String _descriptionPattern = "(?<=meta name=\"description\" content=\").*?(?=\")";
  ...
  private final static Pattern _keyWordsRegexPattern = Pattern.compile(_keyWordsPattern, Pattern.CANON_EQ | Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
  
  private final static Pattern _descriptionRegexPattern = Pattern.compile(_descriptionPattern, Pattern.CANON_EQ | Pattern.DOTALL | Pattern.CASE_INSENSITIVE);
  ...
  public void extractHTML(String htmlText) {
   // System.out.println(htmlText);
    extractMetaData(htmlText);
    htmlText = preProcess(htmlText);
    if( !isContentPage(htmlText) ) {
    	System.out.println("*推测您提供的网页为非主题型网页，目前暂不处理！:-)");
        _text = "";
      return ;
    }
    //System.out.println(htmlText);
   ...
   * @param htmlText the html text
   */
  private void extractMetaData(String htmlText) {
    ...
    m1 = _keyWordsRegexPattern.matcher(htmlText);

    if (m1.find()) {
      _keyWords = replaceSpecialChar(m1.group(0));
    }
    _keyWords = _keyWords.replaceAll("\n+", "");
    
    m1 = _descriptionRegexPattern.matcher(htmlText);

    if (m1.find()) {
      _description = replaceSpecialChar(m1.group(0));
    }
